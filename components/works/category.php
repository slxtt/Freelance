<?php
    ...
    if(isset($_GET['category'])){
        $search = $core->query("SELECT `vacancy`.*, `users`.`id_role` FROM `vacancy` LEFT JOIN `users` ON `vacancy`.`id_user` = `users`.`id` WHERE `id_role` = '1' AND `id_category` = '{$_GET['id_category']}'");
    }
